...
ShapeComponent::ShapeComponent(Ref<Component> parent_, GEOMETRY::Capsule capsule_) :Component(parent_)
{
	shapeType = ShapeType::capsule;
	shape = std::make_shared<GEOMETRY::Capsule>(capsule_.r, capsule_.sphereCentrePosA, capsule_.sphereCentrePosB);
}

ShapeComponent::ShapeComponent(Ref<Component> parent_, GEOMETRY::Box box_) :Component(parent_)
{
	shapeType = ShapeType::box;
	shape = std::make_shared<GEOMETRY::Box>(box_.centre, box_.halfExtents, box_.orientation);
}
...
void ShapeComponent::Render(GLenum drawmode_) const
{
	shape->drawmode = drawmode_;
	shape->debugDraw();
}
