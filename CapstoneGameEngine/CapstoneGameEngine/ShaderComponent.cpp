...
ShaderComponent::ShaderComponent(Ref<Component> parent_, const char* vsFilename_, const char* fsFilename_):
	...
	vsFilename = std::string(vsFilename_);
	fsFilename = std::string(fsFilename_);
}
...
bool ShaderComponent::CompileAttach(){
    GLint status;
	try { 		
		const char* vsText = ReadTextFile(vsFilename.c_str());
		const char* fsText = ReadTextFile(fsFilename.c_str());
